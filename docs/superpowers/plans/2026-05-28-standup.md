...
**Architecture:** Single new file `src/panels/standup.rs`. Two background threads (git scan + session walk) ship results over a shared `mpsc<Msg>` channel. `CalCore` is reused (via its public `days: Vec<DayGroup>`) for meetings, no edits to `src/cal/`. Three pure helpers (`summarize_commits`, `count_sessions`, `summarize_meetings`) are unit-tested; the render function is verified manually.
...
All other code (`commits.rs`, `cal/*.rs`, `theme.rs`) is read-only from this plan. Helpers `project_roots` and `find_repos` are duplicated locally with a `// keep in sync with commits.rs` comment to keep the diff to one file.
...
`1748408400` is the Unix-seconds value of `2026-05-28T00:00:00-05:00`. Verify with `date -d '2026-05-28T00:00:00-05:00' +%s`.
...
```rust
fn day_boundaries(now: jiff::Zoned) -> (SystemTime, SystemTime) {
    let today_civil = now.date().at(0, 0, 0, 0);
    let today_zoned = today_civil
        .to_zoned(now.time_zone().clone())
        .expect("local midnight is always representable");
    let today_secs = today_zoned.timestamp().as_second();
    let today = std::time::UNIX_EPOCH + std::time::Duration::from_secs(today_secs as u64);
    let yesterday = today - std::time::Duration::from_secs(86_400);
    (today, yesterday)
}
```
...
- [ ] **Step 6.1: Add struct, channel, and scan code**
...
```rust
// keep in sync with commits.rs::project_roots
...
// keep in sync with commits.rs::find_repos
...
```
cd ~/projects/glance && git add src/panels/standup.rs && git commit -m "standup: background git scan thread"
```
...
```
cd ~/projects/glance && git add src/panels/standup.rs && git commit -m "standup: CalCore-backed meetings + date rollover"
```
...
fn render(&self, f: &mut Frame, area: Rect) {
    ...
    let meetings_count = format!("{:>3}", total_meetings);
    let urgent = m.next.as_ref().is_some_and(|n| {
        let mins = minutes_until_secs(n.start_secs, now_secs);
        (0..=15).contains(&mins)
    });
    let next_summary = match &m.next {
        Some(n) => format!("{} done · {} left · next {}", m.done, m.upcoming, fmt_hh_mm(n.start_secs)),
        None if total_meetings > 0 => format!("{} done · 0 left", m.done),
        None => String::new(),
    };
    let suffix_style = if urgent { theme::alert() } else { theme::historical() };
    ...
    let next_title_line = match &m.next {
