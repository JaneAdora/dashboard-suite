...
**Recently shipped:**
- ✅ **Reproducible one-liner install + all repos public (2026-06-04).** Published `dashboard-suite`, `glance`, `recall`, `launchers` to public GitHub (all 8 suite repos are now public) after a security audit + adversarial review. `rsuite` now **clones missing component repos**: each `[[launcher]]` in `suite.toml` carries a clone `url`, and `apply.rs` runs `git clone --` (with a validated repo name) instead of skipping, so a fresh Linux box installs the whole suite with one command: `curl --proto '=https' --tlsv1.2 -sSf https://raw.githubusercontent.com/JaneAdora/dashboard-suite/main/install.sh | bash`. `install.sh` is tty-aware (piped goes to `--defaults`). Also: `glance` history was rewritten to scrub sample-mockup PII before publishing, and `cal` now degrades to an "unavailable" message instead of a stuck "loading...". Verified live (real clone+build of roam) plus a fresh-clone PII scan of the public glance.
- ✅ **`suite-term` shared crate + suite-wide migration (2026-06-01)** — extracted the drift-prone clipboard (OSC 52 + wl-copy), shell-quoting, and terminal-restoring panic hook into ONE feature-gated git-dependency crate (https://github.com/JaneAdora/suite-term: `quote` always / `clipboard` / `panic-hook`). Migrated every consumer onto it at one pinned rev `eec40bcd`: launchers (launcher-core re-exports; clip/onepw), glance, recall, roam, wt, atlas (clipboard+quote+panic), mandalas (panic-hook only, pulls no base64 — proving the lean feature-gating). Final cross-cutting review: **0 leftover local copies, all 7 repos build clean (0 warnings).** Fixed for free: wt+recall's previously-uncapped OSC52 now capped at a UTF-8 char boundary; roam+wt gained a panic hook; glance's four standalone bins (cal/crew/tasks/health) gained panic hooks. Built subagent-driven (11 tasks, two-stage review per task); the crate was compile-verified across its full feature matrix (10/11/6/7 tests) before publish. **Closes the shared-crate dedup the 2026-05-29 audit flagged.** Consumer migration commits are local on each repo's `main`; suite-term is pushed/public.
- ✅ **Suite robustness audit (2026-05-29)** — adversarially-verified multi-agent audit (find → independent skeptic-verify) across all 8 repos, hunting the defect classes tests + the compiler miss. Fixed 12 verified bugs: roam OSC52 char-boundary **panic** on `Y` over >4KB UTF-8; subprocess-stderr leaks that corrupt the TUI over SSH (glance tmux spawn; launchers `clip` wl-copy/cliphist + `onepw` wl-copy/auto-clear) — same class as the glance `clip.rs` fix, never propagated; **shell-injection** in `gst` + `wt` emitted `cd` commands (a `.git` dir named `foo; rm -rf ~` ran under `eval`); a **wt stale-group-index TUI crash** on `t`/`r`/delete; an **atlas reverse-search OOB panic**; mandalas preset-param **OOM/hang** + a swallowed save error; the **rsuite clobber guard** silently destroying a user’s same-named ELF binary; recall FTS-error stale-state. All build clean (0 warnings), +regression tests (427 suite-wide green). roam/recall/atlas/launcher-core OSC52 + crew/tasks quoting were checked and already correct.
...
- `launchers` — suite menu tile: a 16-row palette (every launcher + its hotkey) over live cards for `gst`/`clip`/`proc` (each polls `<bin> --summary --json` every 60s). Palette hotkeys copy the launcher name to the clipboard (OSC 52 + wl-copy); `proc`/`note` use `[P]`/`[N]` because lowercase `p`/`n` are the global panel-cycle keys. **Quick-launch shipped 2026-05-29:** a selection cursor (Up/Down/`j`/`k`) + `Enter` spawns the focused launcher in a new tmux window (bare `tmux new-window <name>`, cwd inherits the pane); a per-entry `built` flag gates spawn (built set: gst/clip/1p/proc/roam/wt/recall/mm) so unbuilt rows render dimmed and toast "not built yet"; not-in-tmux falls back to copy. Spawn goes through a shared `src/spawn.rs` helper (`in_tmux`/`tmux_argv`/`tmux_new_window`) that `crew` was refactored onto. We spawn the command name `1p`, never `op` (fork-bomb guard). Follow-up: `eval`-wrap exit-with-command launchers (gst/roam/recall) so their `cd` lands instead of flash-closing.
...
- ✅ **Built 2026-05-26:** `cal` — Google Calendar agenda tile (today + this week). Standalone bin + glance panel. Pure tile: no PrintAndExit (meeting URLs route to the browser, not the terminal). Day-grouped, today auto-expanded with a NOW marker between past and upcoming; future days collapsible like `tasks` session-groups. Keys: `space` copies focused Meet URL via OSC 52, `y` alias, `c` copies full event detail as prompt-pasteable text, `Enter`/`l` opens detail modal with attendees + description + extracted Links footer (catches Otter notes), `Tab`/`o` toggle expand, `h`/`l` drill in/out, `p` toggles past, `r` forces refresh. Past events dim + `✓`; declined events strikethrough; cancelled events hidden. Bridge: shells out to `~/Projects/skai-work/scripts/zele/cal_json.py` (REST API shim shipped 2026-05-26, sidesteps zele's CalDAV path which has an RRULE-expansion bug); 5-min cache at `~/.cache/glance/cal.json` with stale-on-error fallback.
...
2. `curl … | sh` ✅ (2026-06-04) bootstrap that installs the toolchain if needed, clones the meta-repo AND every selected component, runs the installer.
...
3. **Distribution:** source build (phase 1) + `curl … | sh` bootstrap (phase 2, ✅ 2026-06-04) that installs the toolchain if needed, clones the meta-repo + components, runs the picker. Prebuilt/Termux releases (phase 3) deferred.
...
✅ **Reproducible one-liner shipped 2026-06-04.** All 8 suite repos are public on github.com/JaneAdora. Install anywhere with `curl --proto '=https' --tlsv1.2 -sSf https://raw.githubusercontent.com/JaneAdora/dashboard-suite/main/install.sh | bash`. `suite.toml` now carries a clone `url` per component and `rsuite` clones any missing repo (via `git clone --`, repo name validated by `is_safe_repo_name`) instead of skipping it, so a fresh machine gets the whole suite, not just whatever is already under ~/projects. `install.sh` is tty-aware (piped goes to `--defaults`), prechecks git, and warns about both bin dirs. Remaining: prebuilt/Termux releases (phase 3).
